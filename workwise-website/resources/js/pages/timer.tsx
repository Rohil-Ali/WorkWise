    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="flex h-full flex-1 flex-col items-center justify-center gap-4 p-4">
                <h1 className="mb-8 text-6xl font-bold">Timer</h1>
                <div className="flex h-auto w-160 flex-col items-center justify-center rounded-lg bg-gray-100 p-12 shadow-lg">
                    <div className="mb-12 flex gap-16">
                        <button
                            onClick={() => handleModeChange('Pomodoro')}
                            className={`rounded-full px-6 py-3 ${mode === 'Pomodoro' ? 'bg-purple-500 text-white' : 'bg-purple-200'}`}
                        >
                            Pomodoro
                        </button>
                        <button
                            onClick={() => handleModeChange('Short Break')}
                            className={`rounded-full px-6 py-3 ${mode === 'Short Break' ? 'bg-purple-500 text-white' : 'bg-purple-200'}`}
                        >
                            Short Break
                        </button>
                        <button
                            onClick={() => handleModeChange('Long Break')}
                            className={`rounded-full px-6 py-3 ${mode === 'Long Break' ? 'bg-purple-500 text-white' : 'bg-purple-200'}`}
                        >
                            Long Break
                        </button>
                    </div>
